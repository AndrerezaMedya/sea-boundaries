...
CSV_FILE = "real_db_schema/R_ Source_Point.csv"
SQL_OUTPUT_FILE = "real_db_schema/seed_fulocationsource.sql"
...
def generate_sql():
    try:
        with open(CSV_FILE, mode='r', encoding='utf-8-sig') as file, \
            ...
            out_file.write("-- Membuat tabel relasi fmlocation_to_source (fuSource)\n")
            out_file.write("CREATE TABLE IF NOT EXISTS fmlocation_to_source (\n")
            out_file.write("    fuid_location VARCHAR(50) NOT NULL,\n")
            out_file.write("    sID VARCHAR(50) NOT NULL,\n")
            out_file.write("    description TEXT,\n")
            out_file.write("    PRIMARY KEY (fuid_location, sID)\n")
            out_file.write(");\n\n")
            ...
            for row in csv_reader:
                ...
                if fuid and sid:
                    ...
                    insert_stmt = f"INSERT INTO fmlocation_to_source (fuid_location, sID, description) VALUES ('{fuid}', '{sid}', {desc_val}) ON CONFLICT (fuid_location, sID) DO NOTHING;\n"
